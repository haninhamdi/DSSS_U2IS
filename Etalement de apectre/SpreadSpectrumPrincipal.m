...
for N0_bruit = -5:30
    ...
    % Brouillage par balayage 
     ...
     %delta_f = fc/100 ;
     %f = -fc+10*delta_f:delta_f:fc-10*delta_f ; 
    ...
    %X = (brouilleur_gaussien-mean(brouilleur_gaussien))/sqrt(var(brouilleur_gaussien,1));
    %X =fftshift(fft(X)); 
    %X=X.*conj(X)/length(X); 
   ...
   % grid on 
   % legend('brouilleur balayage','signal transmis')
    ...
    TEB_gaus(N0_bruit+6) = Ne_gaus/Nb;
    
    Data_coded_balay = vitdec(Data_estime_balay,trellis,tbdepth,'trunc','hard');
    Ne_balay=sum(abs(data-Data_coded_balay));
    TEB_balay(N0_bruit+6) = Ne_balay/Nb;
    
    Data_coded_rep = vitdec(Data_estime_rep,trellis,tbdepth,'trunc','hard');
    Ne_rep=sum(abs(data-Data_coded_rep));
    TEB_rep(N0_bruit+6) = Ne_rep/Nb;
end

figure 
semilogy([-5:30],TEB_gaus)
hold on
semilogy([-5:30],TEB_balay)
hold on
semilogy([-5:30],TEB_rep)
%title("Le taux d'erreur binaire en fonction de Pb (Pu = 1)") 
xlabel('Pb (dB)')
ylabel('TEB')
