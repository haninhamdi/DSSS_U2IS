...
% phase qui permet d'estimer le déphasage dans le mode Non Data   % 
% aided                                                           %
% Y =[y_1,...,y_N] : Les symboles reçus (complexes)               %
% mu : Le pas adaptatif                                           %
% iterations : le nombre des aller-retour                         %
% N0 : La puissance du bruit du canal (n)                         %
% On commence par beta_F(1)=0                                     %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

function [estimated_phase] = NDA(Y,mu,iterations,N0)
    ...
    for k = 1:iterations 
        ...
        beta_F(1) = beta_B(1);
        for k1 = 2:Nb
            a = Y(k1)*exp(-1i*beta_F(k1-1));
            beta_F(k1) = beta_F(k1-1) + mu*tanh(real(a)/N0)*imag(a) ; 
        end
        % Calcul de beta_B 
        beta_B(Nb) = beta_F(Nb);
        for k2 = 2:Nb
            b = Y(Nb-k2+1)*exp(-1i*beta_B(Nb-k2+2));
            beta_B(Nb-k2+1) = beta_B(Nb-k2+2) + mu*tanh(real(b)/N0)*imag(b);
        end
    end
